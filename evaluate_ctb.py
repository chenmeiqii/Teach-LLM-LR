...
from src.utils import ONE_PAIR_RULES, random_select_consistent_answers_ctb, check_rules_with_engine_double, prepare_rule_text, \
    run_openai_gpt_ctb, \
...
instruction = '''There is a piece of text with two events marked by < and > symbols, and your task is to identify the relations between them. There are four types of relations that need to identify.
(1) Temporal relation. "NO_TEMPORAL": if there is no clear temporal relation between event A and B. "BEFORE": if event A happened completely before event B. "OVERLAP": if event A has an overlap with event B. "CONTAINS": if event A's time contains event B's time. "SIMULTANEOUS": if events A and B happen at the same time. "ENDS-ON": if event A ends when event B starts. "BEGINS-ON": if event A and event B starts at the same time, but ends at different times. 
(2) Causal relation. "NO_CAUSAL": if there is no clear causal relation between event A and B. "CAUSE": if event A causes event B. 
...
def evaluate_events(args, engine):
    train_sample = []
    with open(os.path.join(args.cache_path, "ctb_train_sent_prompts_events.jsonl"), "r", encoding="utf-8") as f0:
        lines = f0.readlines()
        for line in lines:
    ...
    if args.add_rule.startswith('retri') or args.add_rule == 'post':
        ...
        if args.add_rule== 'retri1' or args.add_rule == 'post':
            input_dir = os.path.join("output/42/CTB/", args.model_name,  "test_events_none_res.jsonl")
        else:
            input_dir = os.path.join("output/42/CTB/", args.model_name,
                                 "test_events_" + args.add_rule[:-1] + str(
                                     int(args.add_rule[-1]) - 1) + "_res.jsonl")
    else:
        input_dir = os.path.join(args.cache_path, "ctb_test_sent_prompts_events.jsonl")
    sent_prompts = []
    ...
    if args.add_rule == 'post':
        for sent_i, sent_prompt in enumerate(tqdm(sent_prompts)):
            ...
            new_pred = []
            for p in pred:
                new_pred.append(list(random_select_consistent_answers_ctb([p])))
            for i in range(2):
        ...
        output_path = os.path.join("output/42/CTB/", args.model_name,
...
Reasoning: based on the context and due to the logic rule: if event A CAUSEs event B, then event A happens BEFORE or OVERLAP event B, and they won’t have coreference and subevent relations.
...
\n'''
                if args.add_rule == 'none':
                    ...
                    if len(sent_prompt["retrieved_rule"]) == 0 or (sent_prompt["wrong_num"][0]+sent_prompt["wrong_num"][1]==0) :
                        ...
                        rule_prompt = prepare_rule_text(sent_prompt["retrieved_rule"])
                ...
                if len(batch_samples) == args.batch_size:
                    openai_samples, preds = run_openai_gpt_ctb(batch_prompts, batch_pair_num, args.model_name, args.add_rule, tokenizer=tokenizer, device=args.device, local_model=local_model)
                ...
                if (sent_i == len(sent_prompts) - 1) and len(batch_samples) > 0:
                    openai_samples, preds = run_openai_gpt_ctb(batch_prompts, batch_pair_num, args.model_name,
        ...
        f2.close()
    for i in range(2):
        logging.info(classification_report(labels[i], predictions[i], labels=ALL_RELATION_TYPES_CTB[i][1:], zero_division=0))
...
def main():
    ...
    parser.add_argument("--model_name", default='llama2-13b', type=str, choices=['ChatGPT', 'GPT3', 'GPT4', 'vicuna-13b', 'llama2-13b'])
    ...
    filename = os.path.join(args.output_dir, args.model_name,
                            date_string + "_ctb_events_" + args.add_rule + '_log.txt')
    logging.basicConfig(
        filename=filename, \
